...
from test_p2_best import evaluate
...
if __name__=='__main__':
    ...
    train_loader = torch.utils.data.DataLoader(data.DATA(args, mode='train'),
                                               ...
                                               shuffle=False)
    val_loader   = torch.utils.data.DataLoader(data.DATA(args, mode='validation'),
                                               batch_size=args.train_batch,
                                               num_workers=args.workers,
                                               shuffle=False)
...
#            cls = torch.reshape(cls, (args.train_batch,3, 512, 512)).long()
   #         print(output.shape)
           # print(cls.shape)
        ...
        if epoch%args.val_epoch == 0:
            ...
            #output 3 predicted images in validation set
            if epoch%20 == 5:
                acc = evaluate(epoch, model, val_loader, mode = "validation")
